...
export default function AuthModal({ open, onOpenChange, callbackURL }: AuthModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[340px] p-0 overflow-hidden bg-transparent border-none shadow-none [&>button]:hidden">
        <div className="relative flex flex-col items-center w-full px-4">
          {/* Close button */}
          <DialogClose asChild>
            <button
              className="absolute top-2 right-2 z-20 flex items-center justify-center w-7 h-7 rounded-full bg-card border border-border shadow-sm hover:bg-muted transition-colors"
              aria-label="Close"
            >
          ...
          <div className="relative w-[160px] h-[120px] mb-0 z-10">
            <Image
              className="absolute w-[75px] h-[110px] rounded-2xl shadow-md border-2 border-double object-cover"
              src={loginImg1}
              alt="preview"
              style={{
                bottom: 0,
          ...
          {/* Card */}
          <div
            className="w-full bg-card rounded-2xl shadow-sm px-7 pb-7 flex flex-col items-center gap-4 text-center"
