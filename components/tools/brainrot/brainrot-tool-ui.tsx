...
import { LanguageSelector } from "@/components/shorts/language-selector";
import { TopicDuration } from "@/components/shorts/topic-duration";
import { BgMusicSelector } from "@/components/shorts/bg-music-selector";
import { CaptionConfig } from "@/components/shorts/caption-config";
import AuthModal from "@/components/tools/auth-modal";

import { DialogueScriptSection } from "@/components/conversation-videos/dialogue-script-section";
import { SpeakerAvatarSelector } from "@/components/conversation-videos/speaker-avatar-selector";
import { BackgroundVideoSelector } from "@/components/conversation-videos/background-video-selector";
import { SpeakerVoiceSelector } from "@/components/conversation-videos/speaker-voice-selector";
import { ConversationMockupPreview } from "@/components/conversation-videos/conversation-mockup-preview";
import { Topic } from "@prisma/client";

export default function BrainrotToolUi() {
  ...
  const handleGenerate = () => {
    // Auth gate — show modal if not signed in
    if (!session?.user) {
  ...
  const GenerateButton = (
    <Button
      id="conversation-video-generate-btn"
  ...
  return (
    <div className="w-full flex flex-col font-sans" style={{ height: "950px" }}>
      {/* ── Main card ─────────────────────────────────────────────────────── */}
      <Card
      ...
      {/* Auth Modal — shown if user is not signed in */}
      <AuthModal
        open={showAuthModal}
      ...
      <AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Generate Brainrot Video</AlertDialogTitle>
            <AlertDialogDescription>
